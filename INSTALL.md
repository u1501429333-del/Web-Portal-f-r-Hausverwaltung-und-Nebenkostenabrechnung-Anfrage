...
> **Falls nach dem Login sofort "Keine Berechtigung" / "Fehler beim Laden" erscheint:**
> Das ist ein bereits behobener Bug (Session-Cookie hatte fest `secure: true`, was Browser
> bei einer HTTP-Verbindung wie `http://192.168.178.192:3000` still verwirft) – siehe
> Fehlerbehebung unten ("Nach Login sofort 'Keine Berechtigung'"). Stelle sicher, dass du
> auf dem neuesten `main`-Stand bist (`git pull`, dann Image neu bauen).
...
| Login (Admin **und** Mieter) funktioniert scheinbar, aber danach sofort `Fehler beim Laden` / `Keine Berechtigung` auf jeder Seite (z. B. `/#/admin`) | **War ein eigenständiger Bug im Anwendungscode, unabhängig vom TCMalloc-Problem oben:** Das Session-Cookie wurde bislang fest mit dem Attribut `Secure` gesetzt. `Secure` bedeutet für den Browser "dieses Cookie nur über eine verschlüsselte HTTPS-Verbindung senden" – da das Self-Hosting typischerweise ohne eigenes TLS-Zertifikat über einfaches `http://<lan-ip>:3000` läuft (siehe "Nicht sicher" in der Adressleiste), hat der Browser das Cookie nach dem Login **stillschweigend verworfen**. Jeder Folge-Request erschien dadurch als "nicht angemeldet". **Behoben:** `src/routes/auth.ts` erkennt jetzt automatisch, ob die Verbindung HTTP oder HTTPS ist (bzw. per `X-Forwarded-Proto`-Header hinter einem Reverse-Proxy), und setzt `Secure` nur noch, wenn die Verbindung tatsächlich HTTPS ist. Mit `git pull` + `docker compose build --no-cache && docker compose up -d` (bzw. `bash scripts/update.sh`) aktualisieren, danach im Browser einmal Cookies für die Seite löschen bzw. den Login erneut versuchen. |
