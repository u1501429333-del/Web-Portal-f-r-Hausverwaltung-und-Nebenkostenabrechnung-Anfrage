          ...
          ${mieterListe.map((m) => `
            <div class="border rounded-lg p-3 ${m.aktiv ? 'border-blue-200 bg-blue-50' : 'border-slate-200'}">
              <div class="flex justify-between items-start">
                ...
                <div class="flex gap-2">
                  <button class="text-slate-400 hover:text-blue-600" onclick="openMieterModal(${wohnungId}, ${m.id})"><i class="fas fa-pen"></i></button>
                  <button class="text-slate-400 hover:text-emerald-600" title="Login-Zugang erstellen" onclick="createMieterLoginPrompt(${m.id}, '${escapeHtml(m.email)}')"><i class="fas fa-key"></i></button>
...
function createMieterLoginPrompt(mieterId, email) {
  alert(`Hinweis: Login-Zugänge für Mieter werden derzeit direkt in der Datenbank (Tabelle "users") angelegt bzw. über das Setup-Skript erzeugt. Mieter-ID: ${mieterId}, E-Mail: ${email}`);
}
